...
-- ORDER BY id 
-- I found the answer in the messages in the out put screen
-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 2 >>>>>>>>>>>>>>>>>>>>>>>
...
-- SELECT Avg (availability_365) as AVG_AVAILABILITY_365 FROM final_airbnb;
...
-- SELECT number_of_reviews FROM final_airbnb WHERE number_of_reviews = 0
...
-- SELECT neighbourhood, COUNT(NEIGHBOURHOOD) AS NEIGHBOURHOOD_COUNT FROM final_airbnb GROUP BY neighbourhood ORDER BY NEIGHBOURHOOD_COUNT DESC LIMIT 5;
-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 9 >>>>>>>>>>>>>>>>>>>>>>>
...
-- SELECT id FROM final_airbnb WHERE reviews_per_month  AND minimum_nights< 7 ORDER BY reviews_per_month asc
-- EXPECTED OUTPUT: 58059
